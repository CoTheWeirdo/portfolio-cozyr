...
function IntroBridgeCat({
  ...
  useEffect(() => {
    ...
    // Phone: 105–125px settled footprint; desktop/tablet unchanged.
    ...
    const endW = isPhone
      ? Math.min(125, Math.max(105, vw * 0.28))
      : Math.min(112, Math.max(88, vw * 0.072));
    const endRight =
      (isPhone
        ? Math.min(20, Math.max(12, vw * 0.04))
        : Math.min(40, Math.max(24, vw * 0.024))) + safeRight;
    const endBottom =
      (isPhone
        ? Math.min(24, Math.max(12, vh * 0.025))
    ...
    let cancelled = false;
    let controls: { stop: () => void; then: (fn: () => void) => void } | null = null;
    // Defer one frame so bob transform is fully cleared before Framer writes x/y/scale
    const raf = requestAnimationFrame(() => {
      if (cancelled || !catRef.current) return;
      ...
      controls.then(() => {
        ...
        const visual = elNow.getBoundingClientRect();
